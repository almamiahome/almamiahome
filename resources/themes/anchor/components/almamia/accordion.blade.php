...
<div x-data="{ open: {{ $open ? 'true' : 'false' }} }" {{ $attributes->twMerge('liquid-glass-panel') }}>
    <button type="button" @click="open = !open" class="flex justify-between items-center w-full px-4 py-3 text-left text-primary-900 dark:text-slate-100">
        <span>{{ $title }}</span>
    ...
    <div x-show="open" x-collapse class="px-4 pb-4 text-slate-600 dark:text-slate-300">
        {{ $slot }}
    </div>
</div>