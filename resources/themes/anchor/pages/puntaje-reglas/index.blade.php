                ...
                <div class="flex flex-col lg:flex-row lg:items-center gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <h4 class="mt-2 text-xs font-bold text-slate-800 dark:text-white uppercase truncate">
                            {{ $regla->descripcion ?: 'Regla General' }}
                        </h4>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-6 lg:gap-10 shrink-0 px-4 py-2 lg:py-0 bg-slate-50 dark:bg-slate-800/50 rounded-lg lg:bg-transparent">
                        <div class="text-center">
                            ...
                            <p class="text-xs font-black text-slate-700 dark:text-slate-200">{{ $regla->min_unidades }}-{{ $regla->max_unidades ?? '∞' }}</p>
                        </div>
                        <div class="text-center">
                            ...
                            <p class="text-xs font-black text-indigo-600">
                                {{ $regla->porcentaje ? number_format($regla->porcentaje, 1).'%' : '$'.number_format($regla->bonificacion, 0) }}
                            </p>
                        </div>
                        <div class="text-center">
                            <p class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Mensual</p>
                            <p class="text-xs font-black text-slate-700 dark:text-slate-200">{{ $regla->puntos_mensuales ?? 0 }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 lg:ml-4 border-t lg:border-l lg:border-t-0 border-slate-100 dark:border-slate-800 pt-3 lg:pt-0 lg:pl-4">
                        ...
                        <button
                            type="button"
                            x-data
                            @click="if(confirm('¿Eliminar?')) $wire.deleteRegla({{ $regla->id }})"
                        ...
                        <div class="lg:col-span-2">
                            ...
                            >
                                @foreach($categorias as $categoria)
                                    <option value="{{ $categoria->id }}" class="py-2 px-4 rounded-lg my-1">{{ $categoria->nombre }}</option>
                                @endforeach
                            </select>
                            ...
                            @error('form.categoria_ids') <span class="text-sm text-red-500 font-semibold mt-1">{{ $message }}</span> @enderror
                        </div>
                    </section>
                    ...
                    <section class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            <div>
                                ...
                                ></textarea>
                                @error('form.descripcion') <span class="text-sm text-red-500 font-semibold">{{ $message }}</span> @enderror
                            </div>
                                ...
                                @forelse($form['datos'] as $index => $dato)
                                    <div class="flex items-center gap-4 bg-white dark:bg-slate-800 p-3 rounded-2xl border border-slate-200 dark:border-slate-700 group transition-all hover:border-indigo-300">
                                        <div class="flex-1 grid grid-cols-2 gap-4">
                                            <input type="text" wire:model="form.datos.{{ $index }}.key" placeholder="Ej: Color" class="border-none focus:ring-0 text-sm font-bold text-slate-900 dark:text-white bg-transparent" />
                                            <input type="text" wire:model="form.datos.{{ $index }}.value" placeholder="Valor" class="border-none focus:ring-0 text-sm text-slate-600 dark:text-slate-400 bg-transparent border-l border-slate-100 dark:border-slate-700 pl-4" />
                                        </div>
                                        <button
                                            type="button"
                        ...
                        <div class="lg:col-span-2">
                            <div class="space-y-3">
                                @forelse($form['datos'] as $index => $dato)
                                    <div class="flex items-center gap-4 bg-white dark:bg-slate-800 p-3 rounded-2xl border border-slate-200 dark:border-slate-700 group transition-all hover:border-indigo-300">
                                        <div class="flex-1 grid grid-cols-2 gap-4">
                                            <input type="text" wire:model="form.datos.{{ $index }}.key" placeholder="Propiedad" class="border-none focus:ring-0 text-sm font-bold text-slate-900 dark:text-white bg-transparent" />
                                            <input type="text" wire:model="form.datos.{{ $index }}.value" placeholder="Valor" class="border-none focus:ring-0 text-sm text-slate-600 dark:text-slate-400 bg-transparent border-l border-slate-100 dark:border-slate-700 pl-4" />
                                        </div>
                                        <button type="button" wire:click="removeDatoRow({{ $index }})" class="p-2 text-slate-300 hover:text-red-500 rounded-xl transition-colors">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                        </button>
                                    </div>
                                @empty
