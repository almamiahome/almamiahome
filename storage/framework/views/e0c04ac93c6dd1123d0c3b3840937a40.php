...
<?php $component->withAttributes([]); ?>
    ...
    toggleZoom(path) { 
        ...
        <button class="absolute top-6 right-6 text-white bg-white/10 p-3 rounded-full hover:bg-white/20 transition">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>
        <div class="max-w-5xl w-full h-full flex items-center justify-center" @click.stop>
            <img :src="zoomImage" class="max-w-full max-h-full object-contain rounded-xl shadow-2xl">
        </div>
    </div>
        ...
        <template x-if="paginas.length > 0">
            ...
            >
                <template x-for="(pagina, index) in paginas" :key="pagina.id">
                    <div class="min-w-full snap-start snap-always relative flex justify-center items-center overflow-hidden">
                        <div class="relative w-full aspect-[1061/1500] bg-white">
                            <img :src="pagina.imagen_path" 
    ...
    <div class="flex items-center justify-between gap-4 bg-white p-4 rounded-3xl shadow-sm border border-slate-100 mb-8">
        <div class="flex items-center gap-2">
            <button @click="paginaAnterior()" class="p-3 rounded-2xl bg-slate-50 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 transition-all active:scale-90">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
            </button>
            <div class="px-4 py-2 bg-slate-50 rounded-2xl text-xs font-bold text-slate-500 tracking-widest uppercase">
                <span x-text="paginaActiva + 1"></span> / <span x-text="paginas.length"></span>
            </div>
            <button @click="paginaSiguiente()" class="p-3 rounded-2xl bg-slate-50 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 transition-all active:scale-90">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
            </button>
        </div>

        <button @click="toggleZoom(paginas[paginaActiva].imagen_path)" 
                class="flex items-center gap-2 px-5 py-3 rounded-2xl bg-indigo-600 text-white font-bold text-sm hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all active:scale-95">
...
<style>
    ...
    /* Soporte para navegadores que no leen aspect-ratio en clases arbitrarias de tailwind */
    .aspect-page {
    ...
    <div x-show="!esVendedoraAutenticada && !esLiderAutenticado" x-cloak class="group">
        ...
        <div class="relative">
            ...
            >
                ...
                <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $this->lideres; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div x-show="!esVendedoraAutenticada" x-cloak class="group">
        ...
        <div class="relative">
            ...
            >
                ...
                <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $this->vendedoras; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<div class="bg-white rounded-3xl ring-1 ring-slate-200/60 shadow-sm overflow-hidden mt-8">
    <div class="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-slate-100">
        ...
        <?php echo $__env->make('theme::pages.partials.pedido-app-script', [
...
<?php endif; ?>
    <?php /**PATH /home/unquxtyh/public_html/storage/framework/views/71eabe6fee6be0cd51b4b30b68f6602d.blade.php ENDPATH**/ ?>