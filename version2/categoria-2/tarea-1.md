# Tarea 2.1 — Reglas de saldo a pagar/a cobrar por cierre
...
Reglas de saldo a pagar/a cobrar por cierre. Con ello se contribuye al objetivo macro de la categoría: **Consolidar saldos, deudas, balance y descuentos proyectados a cierres futuros.**.
...
| 5 | `php artisan migrate --pretend` | SQL planificada sin aplicar cambios reales. | Validar que aparecen tablas/columnas esperadas de la tarea **2.1**. | Verificar que el comando termina en exit code 0. |
| 6 | `php artisan test --filter="Tarea2_1"` | Resultado de pruebas relacionadas (`PASS` esperado). | Revisar nombre de pruebas tocadas por la tarea. | Si no existen pruebas específicas, registrar explícitamente `No tests executed` y crear pendiente técnica. |
...
La tarea **2.1** se considera **DONE** únicamente si existe evidencia verificable de:
